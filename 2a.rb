...
input.split("\n").each do |line|
  ...
  rounds       = rounds.split(";").map do |round|
    round.split(",").to_h { |r| r.strip.split.reverse }
  end

  next if rounds.any? do |round|
    round.fetch("red", 0).to_i > 12 ||
    round.fetch("green", 0).to_i > 13 ||
    round.fetch("blue", 0).to_i > 14
  end

  ids += game.match(/(\d+$)/)[1].to_i
end
